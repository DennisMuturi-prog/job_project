      ...
      <div className="relative">
        ...
        >
          ...
          <circle 
            ...
            cy="90" 
            r="15" 
            fill={isSpinning ? "#10B981" : "#3B82F6"}
            stroke={isSpinning ? "#047857" : "#1E40AF"}
            strokeWidth="2"
            onClick={() => !isDoorOpen && setIsSpinning(!isSpinning)}
            className="cursor-pointer transition-colors"
            style={{ cursor: isDoorOpen ? 'not-allowed' : 'pointer' }}
          />
          ...
          {/* Clothes inside drum (only visible when door is closed) */}
          {!isDoorOpen && (
          ...
          <circle
            ...
            cy="280"
            r="110"
            fill={isDoorOpen ? "#1F2937" : "#60A5FA"}
            fillOpacity={isDoorOpen ? "0.1" : "0.3"}
            stroke="#3B82F6"
            strokeWidth="3"
            onClick={() => {
              if (!isSpinning) {
        ...
        <div className="flex gap-4 mt-4">
          ...
          >
            {isSpinning ? 'Stop Drum' : 'Start Drum'}
          </button>
          
          <button
            onClick={() => !isSpinning && setIsDoorOpen(!isDoorOpen)}
            disabled={isSpinning}
            className={`px-6 py-2 rounded-lg transition-colors ${
              isSpinning 
                ? 'bg-gray-400 cursor-not-allowed' 
